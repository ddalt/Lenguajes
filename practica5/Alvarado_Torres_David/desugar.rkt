...
(define (desugar sexpr)
  (type-case SCFWBAE sexpr
    ...
    [withS (bindings body)
         (app (fun (for/list ([i bindings]) (binding-id i)) (desugar body)) (for/list ([i bindings]) (desugar (binding-value i))))]
    
    [withS* (bindings body)
           (if (= (length bindings) 1)
               (app (fun (list (binding-id (car bindings))) (desugar body)) (list (desugar (binding-value (car bindings)))))
               (app (fun (list (binding-id (car bindings))) (desugar (withS* (cdr bindings) body))) (list (desugar (binding-value (car bindings))))))]
